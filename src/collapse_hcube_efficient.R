...
angle_wrap_pi <- function(x) ((x + pi) %% (2*pi)) - pi
...
collapse_objective_givens_delta <- function(
    par,                 # c(delta_angles..., shape_blend)
    n_p,
    theta_init,
    p_inds,
    hcube_init,
  ...
  n_free <- length(p_inds)
  n_delta <- (n_p - 1) * n_free
  
  delta_vec <- par[seq_len(n_delta)]
  shape_blend <- par[n_delta + 1]
  
  delta_mat <- matrix(delta_vec, nrow = n_p - 1, ncol = n_free, byrow = TRUE)
  delta_mat <- angle_wrap_pi(delta_mat)  # keep deltas in (-pi, pi]
  
  # start row is the current angles; subsequent rows are perturbed by delta
  sub_ps <- rbind(theta_init[p_inds], theta_init[p_inds] + delta_mat)
  
  w <- blending_simplex_path(
    k = n_p, n = n_steps,
    shape = 0.8 + invlogit(shape_blend) * 0.4,
    pow_ends = 2
  )
  sub_ps_weighted <- w %*% sub_ps
  ...
  # replace the "abs(coord)-0.5" loss with a direct “axis aligned?” loss
  last_col_median <- median(hcube_end[, k])
  g1 <- which(hcube_end[, k] < last_col_median)
  g2 <- which(hcube_end[, k] >= last_col_median)
  c1 <- colMeans(hcube_end[g1, , drop = FALSE])
  c2 <- colMeans(hcube_end[g2, , drop = FALSE])
  u <- c2 - c1
  u <- u / sqrt(sum(u^2))
  e_k <- c(rep(0, k - 1), 1)
  
  misalign_loss <- (1 - abs(sum(u * e_k)))^2 * loss_weights[1]
  
  # minimal-movement regularizer is now just “keep deltas small”
  dev_loss <- sum(delta_mat^2) * loss_weights[2]
  
  # mild regularization on blend shape (keep near your default)
  shape_blend_loss <- abs(shape_blend + 1)
  
  total_loss <- misalign_loss + dev_loss + shape_blend_loss
  
  if (print_loss) {
    cat("loss:", total_loss,
        "misalign:", misalign_loss,
        "dev:", dev_loss,
        "shape:", shape_blend_loss, "\n")
  }
  total_loss
}
